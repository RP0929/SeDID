...
from mia_evals.model import UNet
import matplotlib
import math
...
def get_FLAGS(flag_path):
    ...
    # Logging & Sampling
    flags.DEFINE_string('logdir', './logs/DDPM_STL10_U_EPS', help='log directory')
    ...
    flags.DEFINE_integer('num_images', 50001, help='the number of generated images for evaluation')
    flags.DEFINE_bool('fid_use_torch', False, help='calculate IS and FID on gpu')
    flags.DEFINE_string('fid_cache', './stats/STL10_U.train.npz', help='FID cache')
    #SeDID
    flags.DEFINE_string('ckpt_sample_path', '/home/mrp_929/projects/SeDID/logs/DDPM_STL10_U_EPS/ckpt-step1190000_samples.pt', help='ckpt_sample_path')
    flags.DEFINE_string('output_folder_path', 'generated_images/DDPM_STL10_U', help='FID cache')
    flags.DEFINE_string('ckpt', '/home/mrp_929/projects/SeDID/logs/DDPM_STL10_U_EPS/ckpt-step80000.pt', help='FID cache')
...
def prepare_generated_dataset(checkpoint_path, output_folder, transform, batch_size=128):
...
def save_matched_diffusions(batch_idx, matched_internal_diffusions, output_folder='matched_diffusions'):
    # 确保输出文件夹存在
    os.makedirs(output_folder, exist_ok=True)

    # 归一化到[0, 1]，如果数据在[-1, 1]
    if matched_internal_diffusions.min() < 0:
        matched_internal_diffusions = (matched_internal_diffusions + 1) / 2
    
    for step, diffusion in enumerate(matched_internal_diffusions):
        # 保存每个时间步的图像
        image_path = os.path.join(output_folder, f'batch_{batch_idx}_step_{step}.png')
        torchvision.utils.save_image(diffusion, image_path)
...
def secmi_stat(model, FLAGS, batch_size=128, score_type='mse'):
    ...
    real_dataset = torchvision.datasets.STL10(root='/home/mrp_929/projects/SeDID/datasets', split='unlabeled',download=True, transform=transforms)
    real_dataset = Subset(real_dataset, range(50000))
    real_loader = DataLoader(real_dataset, batch_size=batch_size, shuffle=False, num_workers=4)
    generated_set = prepare_generated_dataset(FLAGS.ckpt_sample_path, FLAGS.output_folder_path, transforms, batch_size=128)
    generated_loader = DataLoader(generated_set, batch_size=batch_size, shuffle=False, num_workers=4)
    def norm(x):
        return (x + 1) / 2
    ...
    member_scores, member_timestep_diff,member_diffusions, member_internal_samples = get_recon_score(real_loader, ddim_forward_step, ddim_gen_step, score_type=score_type)
...
if __name__ == '__main__':
    fix_seed()
    flag_path = os.path.join("/home/mrp_929/projects/SeDID/STL10_U/flagfile.txt")
    device = 'cuda'
    FLAGS = get_FLAGS(flag_path)
    FLAGS(sys.argv)
    FLAGS.mean_type = 'eps_xt_xt-1'
    FLAGS.T = 1000
    model = get_model(FLAGS.ckpt, FLAGS, WA=True).to('cuda')
