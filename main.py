...
import sys
import setproctitle
setproctitle.setproctitle('mrp_mia_CelebA')
import numpy as np
import warnings
...
from mia_evals.diffusion import GaussianDiffusionTrainer, GaussianDiffusionSampler
...
flags.DEFINE_integer('total_steps', 1800001, help='total training steps')
...
flags.DEFINE_string('dataset', 'CELEBA', help='data set')
# Logging & Sampling
flags.DEFINE_string('logdir', './logs/DDPM_CELEBA_EPS', help='log directory')
...
def get_dataset(FLAGS, only_member=False):
    ...
    if FLAGS.dataset.upper() == 'CIFAR10':
        if FLAGS.defense == 'Cutout':
            ...
            transforms = torchvision.transforms.Compose([torchvision.transforms.RandomHorizontalFlip(),
                                                         ...
                                                         torchvision.transforms.Normalize((0.5, 0.5, 0.5),
                                                                                          (0.5, 0.5, 0.5))])
        if only_member:
            dataset = MIACIFAR10(member_idxs, root='/home/usr/projects/ComputerVisionModels/data', train=True,
                                 transform=transforms, download=True)
        else:
            dataset = CIFAR10(root='/home/usr/projects/ComputerVisionModels/data', train=True,
        ...
        transforms = torchvision.transforms.Compose([torchvision.transforms.RandomHorizontalFlip(),
                                                     ...
                                                     torchvision.transforms.Normalize((0.5, 0.5, 0.5),
                                                                                      (0.5, 0.5, 0.5))])
        if only_member:
            dataset = MIACIFAR100(member_idxs, root='/home/jd3734@drexel.edu/datasets/cifar100', train=True,
                                  transform=transforms)
        else:
            dataset = CIFAR100(root='/home/jd3734@drexel.edu/datasets/cifar100', train=True,
                               transform=transforms)
        ...
        if only_member:
            dataset = MIACelebA(member_idxs, root='/home/usr/projects/data', split='train',
                                transform=transforms, download=False)
        else:
            dataset = CelebA(root='/home/usr/projects/data', split='train',
                             transform=transforms, download=False)
        ...
        transforms = torchvision.transforms.Compose([torchvision.transforms.RandomHorizontalFlip(),
                                                     ...
                                                     torchvision.transforms.Normalize((0.5, 0.5, 0.5),
                                                                                      (0.5, 0.5, 0.5))])
        if only_member:
            dataset = MIASVHN(member_idxs, root='/home/jd3734@drexel.edu/datasets/svhn', split='train',
                              transform=transforms, download=True)
        else:
            dataset = SVHN(root='/home/jd3734@drexel.edu/datasets/svhn', split='train',
                           transform=transforms, download=True)
        ...
        if only_member:
            dataset = MIASTL10(member_idxs, root='/home/jd3734@drexel.edu/datasets/stl10', split='unlabeled',
                               download=True, transform=transforms)
        else:
            dataset = torchvision.datasets.STL10(root='/home/jd3734@drexel.edu/datasets/stl10', split='unlabeled',
                                                 download=True, transform=transforms)
        ...
        if only_member:
            dataset = MIAImageFolder(member_idxs, root='/home/usr/projects/data/tiny-imagenet-200',
                                     transform=transforms)
        else:
            dataset = torchvision.datasets.ImageFolder(root='/home/usr/projects/data/tiny-imagenet-200',
                                                       transform=transforms)
    elif FLAGS.dataset.upper() == 'CIFAR10-SYNTHETIC':
        transforms = torchvision.transforms.Compose([
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize((0.5, 0.5, 0.5),
                                             (0.5, 0.5, 0.5))
        ...
        dataset = torchvision.datasets.ImageFolder(
            root='/home/jd3734@drexel.edu/workspace/pytorch-ddpm-master/experiments/DDPM_CIFAR10_HalfTrain_baseline/member_splits/stylegan_member',
            transform=transforms)
    else:
    ...
    # print(len(data_loader.dataset), FLAGS.dataset)
...
def train():
    dataloader = get_dataset(FLAGS, only_member=False)
    ...
    with trange(FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            if FLAGS.save_step > 0 and step % FLAGS.save_step == 0:
                ...
                torch.save(ckpt, os.path.join(FLAGS.logdir, f'CelebA-ckpt-step{step}.pt'))
